...
const Map: React.FC<MapProps> = ({ onProgressUpdate, progressData, selectedRegion }) => {
    ...
    // 選択されたregionに移動する関数
    const moveToRegion = useCallback((regionId: number) => {
        const feature = geoJSONData?.features.find(f => f.properties?.region === regionId.toString());
    ...
    // selectedRegion が変更されたときに地図を移動
    useEffect(() => {
        if (selectedRegion !== null) {
            moveToRegion(selectedRegion);
        }
    }, [selectedRegion, moveToRegion]);
